...
const Home = () => {
  ...
  const serviceCards = useMemo(() => {
    return productSolutionsData.map((solution) => ({
      ...solution,
    ...
    <>
      ...
      >
        ...
        <div className="relative z-10 mx-auto w-full max-w-6xl px-4 py-16 md:py-20">
          <h1 className="max-w-5xl text-2xl font-semibold leading-[1.1] sm:text-3xl md:text-5xl">
            <span className="block">Powering homes & businesses with smart technology.</span>
            <span className="mt-3 block text-white/95">Internet/Fibre, Networking; CCTV,</span>
            <span className="mt-3 block text-white/95">Electric Fence & Alarm System,</span>
            <span className="mt-3 block text-white/95">Access Control and Time Attendance Systems,</span>
            <span className="mt-3 block text-white/95">Burglar Alarm Systems and Accessories,</span>
            <span className="mt-3 block text-white/95">Networking Products and Accessories,</span>
            <span className="mt-3 block text-white/95">Computer Accessories.</span>
          </h1>
        </div>
      </section>
      ...
      <Section>
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {serviceCards.map((solution) => (
            <Card
              key={solution.id}
              className="group relative flex min-h-[500px] flex-col overflow-hidden border-white/20 bg-transparent p-5 text-white sm:min-h-[460px] sm:p-6"
            >
              <img
            ...
            <div key={product.id} className="min-w-[260px]">
              <Card className="flex h-full flex-col overflow-hidden rounded-none p-0 transition duration-300 hover:-translate-y-1 hover:shadow-lg">
                <div className="overflow-hidden bg-slate-100">
                  <img src={product.image} alt={product.name} className="block h-40 w-full object-cover" />
                </div>
